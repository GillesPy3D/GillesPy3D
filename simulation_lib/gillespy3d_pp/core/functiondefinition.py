...
from gillespy3d_pp.core.error import FunctionDefinitionError
...
class FunctionDefinition(SortableObject):
    ...
    def __init__(self, name="", function=None, args=[]):
        if function is None:
            raise FunctionDefinitionError(
                "Function string provided for FunctionDefinition cannot be None")
    ...
    def sanitized_function(self, species_mappings, parameter_mappings):
        ...
        for i, name in enumerate(names):
            sanitized_function = sanitized_function.replace(
                name, "{" + str(i) + "}")
