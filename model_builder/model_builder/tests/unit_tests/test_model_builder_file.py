...
from model_builder.handlers import GillesPy3DFile
from model_builder.handlers.util.model_builder_errors import GillesPy3DFileNotFoundError, GillesPy3DUnzipError, \
...
class TestGillesPy3DFileObject(unittest.TestCase):
    '''
    ################################################################################################
    Unit tests for the GillesPy3D file class.
    ################################################################################################
    '''
    def setUp(self):
        ''' Create a temporary directory with a file for each file test. '''
        self.tempdir = tempfile.TemporaryDirectory()
        GillesPy3DFile.user_dir = self.tempdir.name
    ...
    def tearDown(self):
        ...
        GillesPy3DFile.user_dir = os.path.expanduser("~")

    ################################################################################################
    # Unit tests for the GillesPy3D file class __init__ function.
    ################################################################################################

    def test_init__new_file__unique(self):
        ''' Check if the new file is written correctly. '''
        GillesPy3DFile(path="test_new_file", new=True)
    ...
    def test_init__new_binary_file__unique(self):
        ...
        GillesPy3DFile(path="test_new_file", new=True, body="test body".encode())
        self.assertIn("test_new_file", os.listdir(self.tempdir.name))


    def test_init__new_file__not_unique(self):
        ''' Check if the new file is writen correctly with a unique name. '''
        GillesPy3DFile(path="test_file", new=True)
        self.assertIn("test_file(1)", os.listdir(self.tempdir.name))

    ################################################################################################
    # Unit tests for the GillesPy3D file class delete function.
    ################################################################################################

    def test_delete(self):
        ''' Check if the file is deleted successfully. '''
        test_file = GillesPy3DFile(path=self.test_filepath)
    ...
    ################################################################################################
    # Unit tests for the GillesPy3D file class duplicate function.
    ################################################################################################
    ...
    def test_duplicate__model_in_new_format_project(self):
        ...
        test_file = GillesPy3DFile(path=test_model_path)
        test_resp = test_file.duplicate()
        self.assertIn("Message", test_resp.keys())
        self.assertIn("File", test_resp.keys())
    ...
    ################################################################################################
    # Unit tests for the GillesPy3D file class move function.
    ################################################################################################
    ...
    ################################################################################################
    # Unit tests for the GillesPy3D file class read function.
    ################################################################################################
    ...
    ################################################################################################
    # Unit tests for the GillesPy3D file class unzip function.
    ################################################################################################
    ...
    def test_unzip__from_upload_true(self):
        ...
        test_file = GillesPy3DFile(path=test_archive)
    ...
    def test_unzip__file_exists_error(self):
        ...
        test_file = GillesPy3DFile(path="test_zip.zip")
    ...
    def test_unzip__bad_zip_file_error__from_upload_true(self):
        ...
        Path(test_file_path).touch()
        test_file = GillesPy3DFile(path=test_file_path)
    ...
    def test_unzip__bad_zip_file_error__from_upload_false(self):
        ''' Check if the GillesPy3DUnzipError is raised when the target file cannot be unzipped. '''
        test_file_path = os.path.join(self.tempdir.name, "test_file.zip")
        Path(test_file_path).touch()
        test_file = GillesPy3DFile(path=test_file_path)
        with mock.patch("zipfile.ZipFile.extractall", mock.mock_open()) as mock_extractall:
            mock_extractall.side_effect = zipfile.BadZipFile
            with self.assertRaises(GillesPy3DUnzipError):
