...
class GillesPy3DSpatialModel(GillesPy3DBase):
    ...
    @classmethod
    def __build_model_builder_domain(cls, s_domain, data=None):
        particles = cls.__build_model_builder_domain_particles(s_domain=s_domain, data=data)
    ...
    @classmethod
    def __build_model_builder_domain_particles(cls, s_domain, data=None):
        particles = []
    ...
    def get_domain_plot(self, path=None, new=False, domains=None):
        ...
        new : bool
            Indicates whether or not to load an new domain
        domains : tuple(spatialpy.Domain, model_builder.Domain)
        ...
        if domains is None:
            ...
            try:
                domain = Domain.read_model_builder_domain(path)
            except DomainError as err:
                raise DomainFormatError(f"Failed to load the domain.  Reason given: {err}") from err
        else:
            domain = domains[0]
            s_domain = domains[1]
        fig_temp_path = "/model_builder/model_builder_templates/domainPlotTemplate.json"
